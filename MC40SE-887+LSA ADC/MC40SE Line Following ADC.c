...
void line_follow_cross(unsigned char cross_count)
{
	...
	while (cross_count > current_cross)
	{
		...
		else if((ADC>=510) && (ADC<=670))	
		{
	   		brushless(PORT2, RUN, CW, 600);	
			brushless(PORT1, RUN, CCW, 400);
			lcd_clear_msg(" right");	
		}	
		else if((ADC>=670) && (ADC<=883))	
		{
			brushless(PORT2, RUN, CW, 600);	
			brushless(PORT1, RUN, CCW, 200);
			lcd_clear_msg(" right1");	
		}
		else if((ADC>=170) && (ADC<=370))
		{
			brushless(PORT2, RUN, CW, 400);	
			brushless(PORT1, RUN, CCW, 600);//turn left
			lcd_clear_msg("left  ");	
		}
		else if(ADC<=170)
		{
		...
		{
			brushless(PORT2, STOP, CW, 0);	
			brushless(PORT1, STOP, CCW, 0);
			lcd_clear_msg("stop  ");	
		}	
				
		//cross count system
		if((SEN1==1))	// if cross sensor detected cross
		{
			if((cross_count - 1) <= current_cross)	 break;	// if this is last cross, break from line following
															// loop and stop robot
			while(SEN1==1) continue;	//wait until the cross sensor is out of line
			current_cross ++;		
		}
