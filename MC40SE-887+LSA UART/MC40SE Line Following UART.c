...
void mc40se_init(void)
{
    ...
    ANSELHbits.ANS12=0; //set RB0 pin as digital output
#endif		
	// clear port value
	PORTA = 0;
	PORTB = 0;
...
void line_follow_cross(unsigned char cross_count)
{
	...
	{
            LF();  //go to line following function
            do{
                current_cross=(unsigned char )LSA08_GetJunction(); //check junction count
            }while(ERR_FLAG||current_cross>10);
            //lcd_2ndline(); lcd_bcd(3,current_cross);
            
	}
        while(LSA08_ClearJunction()!=1);  
...
void LF()
{
    unsigned char UARTRC=0;
    UART_DUMP();
    UARTRC=LSA08_GetSensor();
    //forward movement
    if((UARTRC==0x10) || (UARTRC==0x08) || (UARTRC==0x18))
    {
    ...
    // turn series left
    else if((UARTRC==0x20) || (UARTRC==0x30))
    {
            brushless(PORT2, RUN, CW, 600);
            brushless(PORT1, RUN, CCW, 500);
            lcd_clear_msg("right  ");
    }
    else if((UARTRC==0x40) || (UARTRC==0x60))
    {
            brushless(PORT2, RUN, CW, 600);
            brushless(PORT1, RUN, CCW, 400);
            lcd_clear_msg("right1  ");
    }
    else if((UARTRC==0x80) || (UARTRC==0xC0))
    {
            brushless(PORT2, RUN, CW, 600);
    ...
    //right turn series
    else if((UARTRC==0x04) || (UARTRC==0x0C))
    {
            brushless(PORT2, RUN, CW, 500);
            brushless(PORT1, RUN, CCW, 600);
            lcd_clear_msg("left  ");
    }
    else if((UARTRC==0x06) || (UARTRC==0x02))
    {
            brushless(PORT2, RUN, CW, 400);
            brushless(PORT1, RUN, CCW, 600);
            lcd_clear_msg("left1  ");
    }
    else if((UARTRC==0x01) || (UARTRC==0x03))
    {
